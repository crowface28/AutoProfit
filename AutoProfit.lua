...
rotation = 0;
rotrate = 0;
AUTOPROFIT_VERSION = "v2.6 June 20, 2017";
	...
	for bag = 0, 4 do
		if GetContainerNumSlots(bag) > 0 then
			for slot = 0, GetContainerNumSlots(bag) do
				--DEFAULT_CHAT_FRAME:AddMessage("numOfSales = " .. numOfSales);
				...
				if numOfSales == 5 then break end
			end
		end
	end
end
...
function AutoProfit_SlashCmd(msg)
	...
	if (msg == "") then
		DEFAULT_CHAT_FRAME:AddMessage("|c00bfffffAutoProfit|r " .. AUTOPROFIT_VERSION .. " by Jason Allen.  Patched by Nick for Vanilla Servers.", 0.0, .80, 1);
